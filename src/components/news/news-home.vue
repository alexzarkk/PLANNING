    ...
    <view class="container-wrapper">
        <view id="tabNav" class="container">
            ...
            <view v-for="(item,index) in articleTabs" :key="index">
                <scroll-view v-show="index === currentTab" :scroll-y="isCanScroll || !showMore" class="scroll-view-box" :upper-threshold="0" :lower-threshold="100" :style="listHeight" @scroll="listScroll" @scrolltoupper="scrollInTop" @scrolltolower="loadMore">
                    <view v-if="item&&item.list&&item.list.length > 0">
                        ...
                        <view v-show="showMore && item.list.length > 4" class="response padding bg-white border-bottom" @click="href('/pages/station/news',{tab:index})">
                            <view class="text-center text-blue">查看更多</view>
                        </view>
                        <view v-show="showMore && item.list.length <= 4" class="padding-tb-sm bg-white">
