<template>
 ...
 -->
    <view>
        <view v-for="(item, index) in momentList" :key="index" class="bg-white padding solid-bottom" @click="openDetail(item)">
            ...
            <view class="margin-top-xs push-wrapper padding">
                ...
                <view class="push-content">
                    <zz-upload-image v-if="item.imgs && item.imgs.length > 0" :imgs="item.imgs"></zz-upload-image>
                </view>
            </view>
            <!-- 标签 --> 
        ...
        <zz-page-status
            :loading="loading" :is-center="center" :length="momentList.length" :total="total" msg="当前没有发布动态~"
            btn="去发布"
        ></zz-page-status>
    </view>
</template>
