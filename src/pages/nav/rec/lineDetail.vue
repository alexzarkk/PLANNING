<template>
    ...
    <view>
                ...
                <view class="line-info-box">
                    ...
                    <view class="flex align-center justify-between">
                        ...
                        <view class="padding-right flex">
                            ...
                            <view class="line-operation  radius square" @click="zhouweiShare">
                                    ...
                                    <view class="flex-treble padding-xs radius">
                                        <view class="flex flex-wrap align-center">
                                            <view class="padding-xs" v-for="(x, idx) in rec.ele" :key="idx">
                                                <view class="cu-tag bg-green round lg">{{ dict.trail_element[x].label }}
                                                </view>
		...
		<zz-blog ref="blog" class="solid" :tid="rec._id" :tt="42"></zz-blog>
...
<script>
	...
	export default {
		...
		async onLoad(option) {
			...
			this.loadData()
			uni.$on('newComment' + this.rec._id, () => {
				this.$refs.blog.init()
			})
		},
		methods: {
			loadData() {
				...
				this.lineDetailList[0] = [{
						title: '里程',
						value: `${(info.len / 1000).toFixed(2)}km`
					},
					{
						title: '均速',
						value: `${(info.len / ((r.endTime - r.startTime) / (60 * 60 * 1000)) / 1000).toFixed(2)}km/h`
					}
				]

				this.lineDetailList[1] = [{
						title: '累计海拔',
						value: `⇡${info.up}⇣${info.down}m`
					},
					{
						title: '最高最低',
						value: `⩑${info.top}⩒${info.bottom}m`
					}
				]

				this.lineDetailList[2] = [{
						title: '累计耗时',
						value: this.zz.formatDuring(r.endTime - r.startTime)
					},
					{
						title: '暂停时长',
						value: this.zz.formatDuring(r.stopTime || 0)
					}
				]
			...
			viewImg(p, i) {
				if (p && p.length) this.zz.viewIMG(p, i)
			},
			change3(e) {
			...
			// 跳转到线路编辑
			openEdit() {
				this.zz.href('/pages/nav/rec/edit', {
					kml: this.rec,
					line: this.line,
					point: this.point
				}, 1)
			...
			zhouweiShare() {
				let cur = this.rec;
				let shareData = {
					shareUrl: 'https://zts.5618.co/h5/#/pages/share?path=/pages/nav/rec/lineDetail&id=' + cur._id,
					shareTitle: cur.name,
					shareContent: cur.userInfo.nickName,
					shareImg: 'https://zts.5618.co/repo/shareLogo.png',
					appId: "wx5043dab28d7cf44c",
					type: 0,
					appWebUrl: 'https://zts.5618.co/h5/#/pages/share?path=/pages/nav/rec/lineDetail&id=' + cur._id
				};
				// 调用
