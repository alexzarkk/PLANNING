<template>
	<view class="uni-cursor-point">
		...
		>
			...
			>
				...
				<view
					...
					class="uni-fab__item"
					@touchend="onEndTouch(index, item)"
					@touchstart="onStartTouch(index, item)"
				...
				>
					...
					<!-- item.active ? styles.selectedColor : styles.color -->
		...
		>
			<!-- type="plusempty" -->
			<uni-icons class="fab-circle-icon" :type="icon" :color="styles.iconColor" size="32" :class="{ 'uni-fab__plus--active': isShow && content.length > 0 }"></uni-icons>
...
export default {
	...
	methods: {
		...
		onStartTouch(index, item) {
			this.$emit('fabStartTouch', {
				index,
				item
			});
		},
		onEndTouch(index, item) {
			this.$emit('fabEndTouch', {
				index,
				item
			});
		},
